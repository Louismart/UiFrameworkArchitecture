...
pipelineJob('tests/plutus/plutus-e2e-staging-full-desktop') {
    description('Running plutusTest every day at midnight, desktop view')
    ...
    parameters {
        ...
        stringParam('command', './gradlew -Penv=staging-headless -Ptimeout=30000 -PheavyTimeout=70000 -PmaxFailuresForRetry=100 -PmaxRetries=2 --continue plutusTest allureReport', 'Command to run in a container')
        ...
        choiceParam('kafkaEnvironment', ['tst', 'dev', 'ppd', 'prd', 'shared', 'none'], 'Configure access to kafka environment - as kafka is used in this test suite')
        ...
        stringParam('allureServerProjectName', 'plutus-e2e-staging-full-desktop', 'Name of the project to store it remotely on the allure server')

...
pipelineJob('tests/plutus/plutus-e2e-production-post-deployment-check') {
    description('Running productionPlutusSmokeTest, desktop view')
    ...
    parameters {
        ...
        stringParam('command', './gradlew -Penv=production-headless -PmaxFailuresForRetry=100 -PmaxRetries=2 --continue productionPlutusE2eProductionPostDeploymentCheck allureReport', 'Command to run in a container')
        choiceParam('environment', ['none', 'prd', 'dev', 'tst', 'ppd', 'shared'], 'Configure access to environment. It enables the access to account that hold secrets (the "shared" account should not be confused with "/shared/*" paths of secrets!)')
        ...
        stringParam('slackUsers', '@laap-team-qa, @web-qa-developers', 'Users that will be mentioned in Slack message with Junit test results, like @slacknickname')
        stringParam('slackChannels', 'legacy-api-and-payments-squad/resultChange, web-tests-results/resultChange', 'Slack channel where Junit test results will be sent, separated by comma')
        ...
        stringParam('allureServerProjectName', 'plutus-e2e-production-post-deployment-check', 'Name of the project to store it remotely on the allure server')
